...
export async function getDefinitionsForNode(
  ...
  switch (node.type) {
    case "import_statement": {
      ...
      if (importedModulesNode) {
        importedModules = importedModulesNode.text.match(/\b\w+\b/g);
      }
      else {
        importedModules = undefined;
      }
    
      let resolvedImportedFilePath;
      const stringNode = node.children.find((child) => child.type === 'string');
      if (stringNode) {
        const importFilePath = stringNode.text.slice(1, -1);
        if (!isAbsolutePath(importFilePath)) {
          resolvedImportedFilePath = path.resolve(path.dirname(uri), importFilePath);
          resolvedImportedFilePath = await findClosestMatchingFile(resolvedImportedFilePath) ?? undefined;
        }
        else {
          resolvedImportedFilePath = undefined;
        }
      }
      
      let funDefs:RangeInFileWithContents[] = [];
      if (importedModules && resolvedImportedFilePath) {
        for (let i = 0; i < importedModules.length; i++) {
          const importedModulePath = importedModules[i];
          const contents = await ide.readFile(resolvedImportedFilePath);
          const ast = await getAst(resolvedImportedFilePath, contents); 
          if (ast) {
            let funNodes = findFunctionDefinitions(ast.rootNode, importedModulePath);
            funDefs = funNodes.map((node) => {
              return {
                filepath: importedModulePath,
                range: {
                  start: { line: node.startPosition.row, character: node.startIndex },
                  end: { line: node.endPosition.row, character: node.endIndex },
                },
                contents: node.text,
              };
            });
        ...
        if (resolvedImportedFilePath) {
            const contents = await ide.readFile(resolvedImportedFilePath);
            const ast = await getAst(resolvedImportedFilePath, contents);
            if (ast) {
                let funNodes = findFunctionDefinitions(ast.rootNode, "*");
                funDefs = funNodes.map((node) => {
                    return {
                        filepath: resolvedImportedFilePath,
                        range: {
                            start: { line: node.startPosition.row, character: node.startIndex },
                            end: { line: node.endPosition.row, character: node.endIndex },
                        },
